...
const friendlyLint = (index, objectSomething) => {
  let price = 0;
  for (let key in objectSomething) {
    if (key === index) {
      price = objectSomething[key];
    }
  }
  return price;
};

const calcDrinkPrice = (object) => {
  let drinkPrice = 0;
  for (let index of object.consumption) {
    drinkPrice += friendlyLint(index, object.drink);
  }
  return drinkPrice;
};

const calcFoodPrice = (object) => {
  let foodPrice = 0;
  for (let index of object.consumption) {
    foodPrice += friendlyLint(index, object.food);
  }
  return foodPrice;
};

const createMenu = (baseObject) => {
  if (baseObject === undefined) {
    const object = { food: {}, drink: {} };
    return object;
  }
  const object = {
    food: Object.values(baseObject)[0],
    drink: Object.values(baseObject)[1],
    consumption: [],
    fetchMenu() { return baseObject; },
    order(item) { this.consumption.push(item); },
    pay() { return `R$ ${calcFoodPrice(object) + calcDrinkPrice(object)}`; },
  };
  return object;
};
const restaurant = createMenu({ food: { coxinha: 3.9, sopa: 9.9 }, 
drink: { água: 3.9, cerveja: 6.9 } });
