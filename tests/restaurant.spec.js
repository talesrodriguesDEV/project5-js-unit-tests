...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(Object.keys(test2)[0]).toBe('food');
    expect(Object.keys(test2)[1]).toBe('drink'); 
    ...
    // ```
    const fetchMenu = test2.fetchMenu();
    expect(fetchMenu).toEqual({ food: {}, drink: {} });
    ...
    // ```
    expect(test2.consumption).toEqual([]);
    ...
    // ```
    test2.order('coxinha');
    expect(test2.consumption).toContain('coxinha');
    ...
    test2.order('sopa');
    test2.order('sashimi');
    expect(test2.consumption).toEqual(["coxinha", "água", "sopa", "sashimi"]);
    ...
    // ```
    test2.order('coxinha');
    expect(test2.consumption).toEqual(["coxinha", "água", "sopa", "sashimi", "coxinha"]);
    ...
    // ```
    const test3 = createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'água': 3.9, 'cerveja': 6.9} });
    test3.order('coxinha');
    test3.order('água');
    test3.order('coxinha');
    test3.order('cerveja');
    expect(test3.pay()).toBe(`R$ ${18.6}`);
